...
abstract contract RewardsDistributor is IRewardsDistributor {
  ...
  {
    DataTypes.UserAssetBalance[] memory userAssetBalances = _getUserAssetBalances(
      assets,
      user
    );
  ...
  function _configureAssets(DataTypes.RewardsConfigInput[] memory rewardsInput) internal {
    for (uint256 i = 0; i < rewardsInput.length; i++) {
      ...
      uint256 decimals = _assets[rewardsInput[i].asset].decimals = ERC20(
  ...
  function _updateDataMultiple(
    address user,
    DataTypes.UserAssetBalance[] memory userAssetBalances
  ) internal {
    for (uint256 i = 0; i < userAssetBalances.length; i++) {
      _updateData(
  ...
  function _getPendingRewards(
    address user,
    address reward,
    DataTypes.UserAssetBalance memory userAssetBalance
  ) internal view returns (uint256) {
    DataTypes.RewardData storage rewardData = _assets[userAssetBalance.asset].rewards[
      reward
    ];
  ...
  function _getUserAssetBalances(address[] calldata assets, address user)
    ...
    view
    virtual
    returns (DataTypes.UserAssetBalance[] memory userAssetBalances);
